...
public class AddingMissingComponentWhenNoComponentWasConfigured : TestSpec
{
    ...
    public void When_used__MissingComponent_leaves_an_error_log_in_diagnostics_messages()
    {
        ...
        result?.Messages.ShouldContain(m =>
            m.Level == "error" &&
            m.Message == "`TestPage` doesn't have any component descriptor at path `/page/with-no-config`" &&
            m.Code.HasValue &&
            m.Code.Value.Number == 101 &&
            m.Code.Value.Key == "missing-required-component"
        );
    }
